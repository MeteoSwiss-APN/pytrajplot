...
The only mandatory arguments are ``INPUT_DIR`` and ``OUTPUT_DIR``. The
input directory specifies the path to the source files. In the input
directory, there should be at exactly **one** ``plot_info`` file, and
for each trajectory file one corresponding ``start`` file.
...
1. iterate through the directory and read the start & plot_info files. simultaneously collect all present keys
   *Remark:* The start file is parsed using the ``read_startf`` function and
   the plot_info file is parsed using the ``PLOT_INFO`` class (:file:`pytrajplot/parsing/plot_info.py`).
...
information on the use of each function, see :file:`pytrajplot/parse_data.py`.
...
3. Assembling Ouput: :file:`pytrajplot/generate_pdf.py`
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
:file:`pytrajplot/generate_pdf.py` script iterates through this
...
Every CI pipeline publishes a package to the artifact registry. This allows a separate CD pipeline to deploy the selected artifact.
For ``k8s`` deployments, an extra deployment pipeline must be created using the generated ``Jenkinsfile_k8s_deploy`` file as ``Jenkinsfile``. Then trigger the deployment (deploy with parameters) in Jenkins from the selected branch/tag where the artifact that needs to be deployed has been built (e.g. from main branch or from a released TAG).

For ``acpm`` deployments, the file containing all artifacts in the separate deployment repository must be updated with the version that has been built (either from a branch or from a released TAG).
